...
$(window).on('scroll',function(){
  var scrollEvent = $(window).scrollTop(); //스크롤의 거리
    ...
    if( scrollEvent>=jump5){ 
      $('.product ul li:eq(4) .text_box').addClass('boxMove');
    }
  });
...
function popchange(){
  ...
  if(productData.status === 200){
    ...
    pop_img.setAttribute('src','./images/content1/con2_'+(popIndex+1)+'.jpg');
...
// 이미지 클릭시 팝업띄우기
  $('.product .text_box a').click(function(e){
      ...
      $('.pop .popup img').attr('src','./images/content1/con2_'+(popIndex+1)+'.jpg');
