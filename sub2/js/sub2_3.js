...
productData.open('GET', './data/sub2_3.json', true);// 요청을 준비한다.
productData.send(null);                         // 요청을 전송한다.
  ...
  $('.content1 .img_box ul li a').click(function(e){
   ...
   $("html,body").stop().animate({"scrollTop":jump},500); //스크롤을 스무스하게 움직이는 코드
  });
  ...
  var scrollEvent = $(window).scrollTop(); //스크롤의 거리
    ...
    if( scrollEvent>=jump4){
      $('.product ul li:eq(3) .text_box').addClass('boxMove');
    }
  })
  ...
  function popchange(){
    ...
    if(productData.status === 200){
      ...
      pop_img.setAttribute('src','./images/content3/con2_'+(popIndex+1)+'.jpg');
